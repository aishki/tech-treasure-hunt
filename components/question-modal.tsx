  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
      ...
      <div className="bg-gray-900/80 border border-cyan-700 rounded-lg w-full max-w-md p-6 shadow-[0_0_30px_rgba(0,255,255,0.3)] relative backdrop-blur-sm">
        ...
        <button onClick={onClose} className="absolute top-2 right-2 text-cyan-400 hover:text-white">
          <X size={20} />
        </button>
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-bold text-cyan-300 glow-text font-orbitron">Sector #{boxNumber}</h2>
          </div>

          <div className="text-sm">
            <div className="text-cyan-500 mb-1 font-orbitron">Attempts Remaining</div>
            <div className="flex space-x-1">
              {[...Array(maxAttempts)].map((_, i) => (
                <div
                  key={i}
                  className={`w-3 h-3 rounded-full ${i < remainingAttempts ? "bg-cyan-400" : "bg-gray-700"}`}
                ></div>
              ))}
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <input
              ...
              className={`w-full p-3 bg-black/70 border ${error ? "border-red-500/70" : "border-cyan-700/70"} rounded text-white placeholder:text-cyan-700/70 focus:outline-none focus:ring-2 focus:ring-cyan-500/70 focus:border-transparent`}
              autoFocus
            />
            ...
            {error && (
              <div className="mt-2 text-red-400 flex items-center shake-animation">
                ...
                <span className="text-sm font-orbitron">Wrong answer. Try again.</span>
              </div>
            )}
          </div>
