...
export default function TreasureHunt() {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      try {
        const { data, error } = await supabase.from("hunt_questions").select("*").order("question_id")
  ...
  useEffect(() => {
    ...
    if (Object.keys(boxStatus).length > 0) {
      ...
      // Update stats
      const correct = Object.values(boxStatus).filter((box) => box.status === "correct").length
      const failed = Object.values(boxStatus).filter((box) => box.status === "failed").length
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <div className="animate-pulse text-cyan-400 font-orbitron">Loading treasure hunt...</div>
      </div>
    )
  }
  ...
  return (
    <div className="relative">
      <div className="mb-6 p-4 bg-black/30 border border-cyan-800/30 rounded-lg backdrop-blur-sm">
        <h2 className="text-xl font-bold text-cyan-300 mb-3 glow-text font-orbitron">Traveller Progress</h2>
        <div className="flex flex-wrap gap-4">
          <div className="flex items-center bg-black/50 p-3 rounded-md border border-cyan-800/50">
            ...
            <div>
              <div className="text-xs text-cyan-500 font-orbitron">Discovered</div>
              <div className="text-xl font-bold text-yellow-400">{stats.correct}</div>
            </div>
          </div>
          ...
          <div className="flex items-center bg-black/50 p-3 rounded-md border border-cyan-800/50">
            ...
            <div>
              <div className="text-xs text-cyan-500 font-orbitron">Remaining</div>
              <div className="text-xl font-bold text-cyan-400">{stats.remaining}</div>
            </div>
          </div>
        </div>
      </div>
